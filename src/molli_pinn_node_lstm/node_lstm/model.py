...
class ModelParametersProjection(nn.Module):
    ...
    def __init__(self, latent_dim: int, output_dim: int = 3) -> None:
        ...
        self.softplus = nn.Softplus(
            beta=0.5
        )  # beta = 0.5 to make it smoother in final estimates.
...
class NeuralODELSTM(nn.Module):
    ...
    def forward(
        ...
        if tvec.ndim != 1:
            raise ValueError(
                f"tvec must have shape (seq_len,), got {tuple(tvec.shape)}"
            )
...
class MOLLINeuralODELSTM(nn.Module):
    ...
    def __init__(
        ...
        self.bloch_head = ModelParametersProjection(
            latent_dim=lstm_hidden_size, output_dim=3
        )
        self.register_buffer("k_offset", torch.tensor(2.0))

    def forward(
        self, volume: torch.Tensor, tvec: torch.Tensor, **kwargs
        ...
        if volume.ndim != 2:
            raise ValueError(
                f"volume must have shape (batch, seq_len); got {tuple(volume.shape)}"
            )
        if tvec.ndim != 1:
            raise ValueError(
                f"tvec must have shape (seq_len,); got {tuple(tvec.shape)}"
